...
import {parseCarBookingPayload} from "@/features/cars/lib/schemas";
import {locales} from "@/lib/i18n/routing";
import {createSupabaseServerClient} from "@/lib/supabase/server";
import {resolveBookingCustomerUserId} from "@/server/customer-access/guest-checkout";
import {createPendingCarBooking} from "@/server/cars/booking-service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cachedOffer = await getCachedCarOffer(bodyMeta.offerId);

    if (!cachedOffer) {
      return NextResponse.json(
        {
          message: "The selected rental offer is no longer available."
        },
        {status: 404}
      );
    }

    const payload = parseCarBookingPayload(rawBody);
    ...
    const confirmation = await createPendingCarBooking({
    ...
    if (error instanceof ZodError) {
      return NextResponse.json(
        {
          message: error.issues[0]?.message ?? "Invalid car booking request."
        },
        {status: 400}
      );
    }
