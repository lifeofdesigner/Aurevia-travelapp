...
import {initializeKorapayCheckoutForOwnedBooking} from "@/server/payments/korapay-service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const checkout = await initializeKorapayCheckoutForOwnedBooking({
    ...
    if (error instanceof ZodError) {
      const response = NextResponse.json(
        {
          message: error.issues[0]?.message ?? "Invalid Korapay checkout request."
        },
        {status: 400}
      );
      applyPrivateRouteHeaders(response.headers);
    ...
    reportServerError("payments.korapay.initialize_failed", error);
    const response = NextResponse.json(
      {
        message: error instanceof Error ? error.message : "Unable to initialize Korapay checkout."
      },
      {status: 500}
    );
    applyPrivateRouteHeaders(response.headers);
